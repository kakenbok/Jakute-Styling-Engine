package features.displaylist {
	...
	public class MovingDisplayObject4 extends Sprite {
		
		private var _childBox : Box;
		private var _lastParent : Box;
		private var _currentDropTarget : Box;
		
		public function MovingDisplayObject4() {
			JCSS_Sprite.jcss = new JCSS();
			JCSS_Sprite.jcss.setStyleSheet(MovingDisplayObject4CSS.styles);
			...
			// yellow box
			var yellowBox : Box = new Box();
			yellowBox.jcss_cssID = "yellow";
			yellowBox.jcss_cssClass = "container";
			addChild(yellowBox);

			// green box
			var greenBox : Box = new Box();
			greenBox.jcss_cssID = "green";
			greenBox.jcss_cssClass = "container";
			addChild(greenBox);
		...
		private function checkDropTarget(stageX : Number, stageY : Number) : void {
			...
			if (_currentDropTarget != target) {
				...
				// set target styles
				var boxX : Number = _childBox.x;
				var boxY : Number = _childBox.y;
				_childBox.jcss_virtualParent = _currentDropTarget;
				_childBox.x = boxX;
				_childBox.y = boxY;
			}
			_currentDropTarget.jcss_setState("over", "true");
